...
public class addThread extends automatThread
{
    ...
    public static Kuchen getRandomKuchen(int art) throws IllegalArgumentException
    {
        ...
        int fachnummer = ThreadLocalRandom.current().nextInt(0,99999999);
        Hersteller hersteller = new Hersteller(UUID.randomUUID().toString());
        ...
        if(art == 1)
        {
            k = new Kuchen(preis, inspektionsdatum, fachnummer, hersteller, naehrwert, allergene, haltbarkeit);
            return k;
        }
        else if(art == 2)
        {
            k = new Kremkuchen(preis, inspektionsdatum, fachnummer, hersteller, naehrwert, allergene, haltbarkeit, kremsorte);
            return k;
        }
        else if(art == 3)
        {
            k = new Obstkuchen(preis, inspektionsdatum, fachnummer, hersteller, naehrwert, allergene, haltbarkeit, obstsorte);
            return k;
        }
        else if(art == 4)
        {
            k = new Obsttorte(preis, inspektionsdatum, fachnummer, hersteller, naehrwert, allergene, haltbarkeit, kremsorte, obstsorte);
            return k;
        }
        else
            return k = new Kuchen();
